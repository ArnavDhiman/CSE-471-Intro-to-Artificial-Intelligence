...
class ValueIterationAgent(ValueEstimationAgent):
            ...
            if self.mdp.isTerminal(state):
              ...
              for action in actions:
                ...
                for nextState in nextStates:
                  ...
                  value = oldVal[nextState[0]]
                  probability = nextState[1]

                  qVal += probability*(reward+(discount*value))
    ...
    def computeQValueFromValues(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        #print(self.getTransitionStatesAndProbs(state, action))
        qVal = 0
        ...
        for nextState in nextStates:
          reward = self.mdp.getReward(state, action, nextState[0])
          discount = self.discount
          value = self.values[nextState[0]]
          probability = nextState[1]

          qVal += probability*(reward+(discount*value))
          #print("reward = ", reward, "discount = ", discount, "value = ", value, "probability = ", probability)
        return qVal
...
class PolicyIterationAgent(ValueEstimationAgent):
      ...
      for state in totalStates:
        if not self.mdp.isTerminal(state):
          ...
          for nextState in nextStates:
            ...
            value = oldVal[nextState[0]]
            probability = nextState[1]
            v += probability*(reward+(discount*value))
    ...
    def runPolicyIteration(self):
      # Write value iteration code here
      "*** YOUR CODE HERE ***"
      policy = self.runPolicyInit()
      policyStable = False

      while not policyStable:
        
        policyStable = True
        val = self.runPolicyEvaluation(policy = policy)
        totalStates = self.mdp.getStates()
        #print(val)
        ...
        for state in totalStates:
          ...
          for action in actions:
            ...
            for nextState in nextStates:
              ...
              reward = self.mdp.getReward(state, action, nextState[0])
              discount = self.discount
              value = val[nextState[0]]
              probability = nextState[1]

              t += probability*(reward+(discount*value))
