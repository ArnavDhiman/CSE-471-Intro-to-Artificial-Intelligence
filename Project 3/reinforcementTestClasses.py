...
class PolicyIterationTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        agent = moduleDict['valueIterationAgents'].PolicyIterationAgent(self.grid, discount=self.discount)
        states = self.grid.getStates()
        result = True
        for state in states:
            if not self.grid.isTerminal(state):
                if eval("\'{}\' not in {}".format(agent.policy[state], solutionDict[str(state)])):
                    self.addMessage('Policy differs at state: {}'.format(state))
                    self.addMessage('Student policy: {}'.format(agent.policy[state]))
                    self.addMessage('Correct policy: {}'.format(solutionDict[str(state)]))
                    result = False
        if result:
