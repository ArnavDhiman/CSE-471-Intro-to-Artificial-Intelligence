...
import re
import copy
from bayesNet import CPT, BayesNetwork
from util import manhattanDistance

fixed_order = ['West', 'East', 'Stop', 'South', 'North']

class SingleInferenceTest(testClasses.TestCase):
    def __init__(self, question, testDict):
        super(SingleInferenceTest, self).__init__(question, testDict)
        self.nodes = eval(testDict["nodes"])
        network = dict()
        for node in self.nodes:
            array = eval(testDict[node])
            numDependencies = array[0]
            dependencies = []
            probabilities = []
            for i in range(1, numDependencies + 1):
                dependencies.append(array[i])
            for i in range(numDependencies + 1, len(array)):
                probabilities.append(array[i])
            network[node] = CPT(dependencies, probabilities)
        self.A = testDict["solveFor"]
        observation = eval(testDict["given"])
        self.B = (observation[0], observation[1])
        self.agent = BayesNetwork(network)

    def execute(self, grades, moduleDict, solutionDict):
        answer = float(solutionDict["solution"])
        studentAnswer = self.agent.singleInference(self.A, self.B)
        if abs(studentAnswer - answer) < (answer / 10):
            return self.testPass(grades)
        else:
            self.addMessage('Solution is different.\nExpected: ')
            self.addMessage(str(answer))
            self.addMessage('Student answer: ')
            self.addMessage(str(studentAnswer))
            return self.testFail(grades)


class MultipleInferenceTest(testClasses.TestCase):
    def __init__(self, question, testDict):
        super(MultipleInferenceTest, self).__init__(question, testDict)
        self.nodes = eval(testDict["nodes"])
        network = dict()
        for node in self.nodes:
            array = eval(testDict[node])
            ...
            network[node] = CPT(dependencies, probabilities)
        self.A = testDict["solveFor"]
        preObserve = eval(testDict["given"])
        self.observations = []
        for i in range(0, len(preObserve), 2):
            self.observations.append((preObserve[i], preObserve[i + 1]))
        self.observations = eval(testDict["given"])
        self.agent = BayesNetwork(network)

    def execute(self, grades, moduleDict, solutionDict):
        answer = float(solutionDict["solution"])
        studentAnswer = self.agent.multipleInference(self.A, self.observations)
        if abs(studentAnswer - answer) < (answer / 10):
            return self.testPass(grades)
        else:
            self.addMessage('Solution is different.\nExpected: ')
