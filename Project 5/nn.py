...
def as_vector(node):
    """
    Returns the value of a Node as a standard Python list. This only works
    for nodes with > one element.
    """

    assert isinstance(node, Node), (
        "Input must be a node object, instead has type {!r}".format(
            type(node).__name__))
    #print(list(node.data.flatten()))
    return list(node.data.flatten())
