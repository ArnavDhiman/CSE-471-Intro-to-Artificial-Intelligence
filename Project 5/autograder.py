...
def check_logreg(tracker):
    ...
    for dimensions in range(1, 11):
        ...
        if np.isclose(score, expected_score):
            ...
            print("The score computed by LogisticRegressionModel.run() ({:.4f}) does not match the expected score ({:.4f})".format(score, expected_score))
    ...
    accuracy = np.mean(np.where(np.dot(dataset.x, np.array(model.get_weights()).flatten()) >= 0.0, 1.0, -1.0) == dataset.y.T)
    if accuracy < 1.0:
...
def check_regression(tracker):
    import models
    model = models.RegressionModel(1)
    ...
    #print(list(dataset.x.flatten()))
    b0,b1 = model.closedFormSolution(list(dataset.x.flatten()), list(dataset.y.flatten()))
    x = dataset.x.flatten()
    y = dataset.y.flatten()
    A = np.vstack([x, np.ones(len(x))]).T
    m, c = np.linalg.lstsq(A, y, rcond=None)[0]
    if (np.isclose(b0, c) and np.isclose(b1, m)):
...
def check_polyregression(tracker):
    ...
    for batch_size in (1, 1, 1):
        ...
        output_node = model.run(inp_x)
        verify_node(output_node, 'node', (batch_size, 1), "PolyRegressionModel.run()")
        trace = trace_node(output_node)
    ...
    # to always return zero
    train_predicted = model.run(nn.Constant(dataset.x))
    verify_node(train_predicted, 'node', (dataset.x.shape[0], 1), "PolyRegressionModel.run()")
    sanity_loss = 0.5 * np.mean((train_predicted.data - dataset.y)**2)
...
def check_fashion_classification(tracker):
    ...
    for batch_size in (1, 2, 4):
        ...
        inp_y = nn.Constant(dataset.y[:batch_size])
        output_node = model.run(inp_x)
        verify_node(output_node, 'node', (batch_size, 10), "FashionClassificationModel.run()")
        trace = trace_node(output_node)
    ...
    for batch_size in (1, 2, 4):
        ...
        inp_y = nn.Constant(dataset.y[:batch_size])
        loss_node = model.get_loss(inp_x, inp_y)
        verify_node(loss_node, 'loss', None, "FashionClassificationModel.get_loss()")
        trace = trace_node(loss_node)
