...
def getNoisyAction(pos1, pos2):
    x1, y1 = pos1
    x2, y2 = pos2
    actionSuccess = random.random()
    if x2 - x1 > 0:
        if actionSuccess < 0.6:
            return Directions.EAST
        else:
            newAction = random.random()
            if newAction < 0.25:
...
def getObservationProbability(noisyAction, oldGhostPos, newGhostPos):
    """
    Returns the probability P( noisyAction | oldGhostPost, newGhostPos)
    """
    if oldGhostPos[1] - newGhostPos[1] > 0:
        if noisyAction == Directions.NORTH:
            return 0.6
        else:
            return 0.4
    elif oldGhostPos[1] - newGhostPos[1] < 0:
        if noisyAction == Directions.SOUTH:
            return 0.5
        else:
            return 0.5
    elif oldGhostPos[0] - newGhostPos[0] > 0:
        if noisyAction == Directions.EAST:
            return 0.7
        else:
            return 0.3
    elif oldGhostPos[0] - newGhostPos[0] < 0:
        if noisyAction == Directions.WEST:
            return 0.85
        else:
            return 0.15
    else:
        if noisyAction == Directions.STOP:
            return 0.9
        else:
            return 0.1
...
class GameState:
    ...
    def getResult( self, agentIndex, action):
        ...
        if agentIndex == 0:  # Pacman is moving
            ...
            if action != None:
                ...
                PacmanRules.applyAction( state, Directions.STOP)
        ...
        state.data.ghostActions = [getNoisyAction(self.getPrevGhostPos(), state.getGhostPosition(i)) for i in range(1,state.getNumAgents())]
    ...
    def getNoisyGhostActions(self):
        """
        Returns a noisy distance to each ghost.
        """
        return self.data.ghostActions
    ...
    def __init__( self, prevState = None ):
        ...
        if prevState != None:
            ...
            self.prevGhostPos = prevState.data.agentStates[1].getPosition()
        else: # Initial state
            self.data = GameStateData()
            self.numMoves = 0;
            self.maxMoves = -1;
            self.prevGhostPos = (0,0)
        self.data.ghostActions = []

    def deepCopy( self ):
        state = GameState( self )
        state.data = self.data.deepCopy()
        state.data.ghostActions = self.data.ghostActions
        return state
    ...
    def initialize( self, layout, numGhostAgents=1000 ):
        ...
        self.data.ghostActions = [getNoisyAction(self.getPrevGhostPos(), self.getGhostPosition(i)) for i in range(1, self.getNumAgents())]
    ...
    def getPrevGhostPos(self):
        return self.prevGhostPos
